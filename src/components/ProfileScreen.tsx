...
import { LogOut, Shield, Settings, Bell, HelpCircle, ChevronRight, User, Star, MapPin, Moon, Sun, X, Briefcase, Building2, Map, Plus, CheckCircle2 } from 'lucide-react';
...
const ProfileScreen = ({ 
  ...
  notificationSettings: { enabled: boolean; offset: number; notifyNormal: boolean; notifyExtra: boolean },
  setNotificationSettings: (val: { enabled: boolean; offset: number; notifyNormal: boolean; notifyExtra: boolean }) => void
}) => {
  ...
  const [tempNotifEnabled, setTempNotifEnabled] = useState(notificationSettings.enabled);
  const [tempNotifOffset, setTempNotifOffset] = useState(notificationSettings.offset);
  ...
  useEffect(() => {
    ...
    setTempNotifEnabled(notificationSettings.enabled);
    setTempNotifOffset(notificationSettings.offset);
    setTempNotifyNormal(notificationSettings.notifyNormal);
    setTempNotifyExtra(notificationSettings.notifyExtra);
  }, [workplace, extraLocations, profession, notificationSettings]);
  ...
  const handleSaveSettings = async () => {
    ...
    setNotificationSettings({
      enabled: tempNotifEnabled,
      offset: tempNotifOffset,
      notifyNormal: tempNotifyNormal,
      notifyExtra: tempNotifyExtra
    });
                ...
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-600 dark:text-slate-400">
                      {activeTab === 'profile' ? <Settings size={20} /> : <Bell size={20} />}
                    </div>
                    <h3 className="font-headline font-black text-xl text-slate-900 dark:text-white tracking-tight">
                      {activeTab === 'profile' ? 'Configurações de Perfil' : 'Alertas e Notificações'}
                    </h3>
                  </div>
                  <button 
                    onClick={() => setIsSettingsOpen(false)}
                  ...
                  {activeTab === 'profile' ? (
                    ...
                    <>
                      ...
                      <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="font-bold text-slate-700 dark:text-slate-200 block">Ativar Alertas</span>
                              <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wider">Lembretes de plantão</span>
                            </div>
                          </div>
                          <button 
                      ...
                      {tempNotifEnabled && (
                        ...
                        >
                          <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Horário do Alerta</label>
                            <div className="grid grid-cols-2 gap-2">
                              {[
                                ...
                                <button
                                  key={opt.value}
                                  onClick={() => setTempNotifOffset(opt.value)}
                                  className={`py-3 rounded-xl font-bold text-[11px] border-2 transition-all ${
                                    tempNotifOffset === opt.value 
                                      ? 'border-secondary bg-blue-50 dark:bg-blue-900/20 text-secondary' 
                          ...
                          <div className="space-y-3">
                            <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] ml-1">Tipos de Plantão</label>
                            <div className="space-y-2">
                              <button 
                                onClick={() => setTempNotifyNormal(!tempNotifyNormal)}
                                className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${tempNotifyNormal ? 'border-primary bg-primary/5 text-primary' : 'border-slate-100 dark:border-slate-800 text-slate-400'}`}
                              >
                                <span className="font-bold text-sm">Plantões Normais</span>
                                {tempNotifyNormal ? <CheckCircle2 size={18} /> : <div className="w-[18px] h-[18px] rounded-full border-2 border-slate-200 dark:border-slate-700" />}
                              </button>
                              <button 
                                onClick={() => setTempNotifyExtra(!tempNotifyExtra)}
                                className={`w-full flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${tempNotifyExtra ? 'border-warning bg-warning/5 text-warning' : 'border-slate-100 dark:border-slate-800 text-slate-400'}`}
                              >
                                <span className="font-bold text-sm">Plantões Extras</span>
                                {tempNotifyExtra ? <CheckCircle2 size={18} /> : <div className="w-[18px] h-[18px] rounded-full border-2 border-slate-200 dark:border-slate-700" />}
                              </button>
                            </div>
                          </div>
                        </motion.div>
  ...
  return (
    ...
    >
      {/* Profile Card */}
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.2)] text-center relative overflow-hidden border-none text-slate-900 dark:text-white">
        <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-br from-primary-dark to-secondary opacity-5"></div>
        
        <div className="relative z-10">
          <div className="w-24 h-24 rounded-full bg-slate-50 dark:bg-slate-800 mx-auto mb-4 border-4 border-white dark:border-slate-900 shadow-lg overflow-hidden">
            <img 
          ...
          <h2 className="font-headline text-2xl font-black text-slate-900 dark:text-white mb-1 tracking-tighter uppercase">
            {user?.displayName || "Oficial Silva"}
          </h2>
          <p className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-widest mb-4 leading-none opacity-60">ID: {user?.uid.substring(0, 8) || "GUEST"}</p>
          
          <div className="flex justify-center gap-3">
            <div className="flex items-center gap-1.5 text-[9px] font-black bg-primary/10 text-primary px-4 py-1.5 rounded-xl uppercase tracking-widest">
              <Star size={12} fill="currentColor" />
              <span>{profession || "Sargento"}</span>
            </div>
            <div className="flex items-center gap-1.5 text-[9px] font-black bg-slate-50 dark:bg-slate-800 text-slate-500 px-4 py-1.5 rounded-xl uppercase tracking-widest">
              <MapPin size={12} />
      ...
      {/* Menu List */}
      <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-[0_8px_30px_rgba(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.2)] overflow-hidden">
        {menuItems.map((item, index) => {
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 ${item.bg} ${item.darkBg} rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110`}>
                  <Icon size={22} className={item.color} />
                </div>
                <span className="font-black text-slate-700 dark:text-slate-200 text-sm uppercase tracking-tight">{item.label}</span>
              </div>
          ...
          <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-4">
            ...
            >
              ...
              <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto no-scrollbar">
                ...
                <div className="pt-4 sticky bottom-0 bg-white dark:bg-slate-950 pb-2">
                  <button 
                    onClick={handleSaveSettings}
                    className="w-full bg-gradient-to-r from-primary-dark to-secondary text-white py-4 rounded-2xl font-black uppercase tracking-widest text-sm shadow-xl shadow-primary-dark/20 active:scale-95 transition-all"
                  >
